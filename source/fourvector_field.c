...
void SU3_calculate_A(double complex * U, pos_vec position, int mu, double complex * A){
	...
	SU3_hermitean_conjugate(get_link(U, position, mu), U_dagger);
	
	//	(U - U_dagger)/2i 
	SU3_subtraction(get_link(U, position, mu), U_dagger, U_minus_Udagger);
...
// 						SU3_update_A_local(U,G,A,position,mu);
